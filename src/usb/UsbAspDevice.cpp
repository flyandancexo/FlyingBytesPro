                              ...
                              QString& error)
{
    ...
    output.resize(length);
    const int frequency = usbasp::clockFrequencyHz(clock);
    // At fixed SCK rates below 10 kHz, keep each control-IN data stage to
    // one low-speed USB packet. Physical 500 Hz and 1 kHz EEPROM verify
    // testing showed that the former 20-byte multi-packet reads could return
    // a zero-length transfer even while 4-byte EEPROM writes were reliable.
                               ...
                               QString& error)
{
    ...
    const int frequency = usbasp::clockFrequencyHz(clock);
    // At fixed SCK rates below 10 kHz, keep each control-OUT data stage to
    // one low-speed USB packet. This avoids long multi-packet WRITEFLASH /
    // WRITEEEPROM control transfers while the USBasp firmware is servicing
    // software-SPI bytes at 500 Hz, 1 kHz, 2 kHz, and similar rates.
