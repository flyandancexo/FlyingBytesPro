...
def main() -> int:
    ...
    if "project(FlyingBytesPro" not in cmake or "VERSION 3.2.28" not in cmake:
    ...
    for code in sorted(expected_languages):
        ...
        if pack.get("language") != code or len(strings) != 1131:
    ...
    require(technical, (
        "# FlyingBytesPro V3.2.28 — Technical Reference and Development History",
        "The next development version is **V3.2.29**.",
    ...
    if "FlyingBytesPro" not in main_cpp or "V3.2.28" not in main_cpp:
    ...
    require(main_window, (
        "FlyingBytesPro V3.2.28", "Task", "Flash", "EEPROM", "Full Log",
    ...
    ), "V3.2.28 MCU ID, Settings defaults/language, and compact language status")
    require(main_window, (
    ...
    restore_pos = main_window.find('layout->addWidget(restoreBox);', language_pos)
    stretch_pos = main_window.find('layout->addStretch();', restore_pos)
    if (signature_pos < 0 or language_pos < 0 or restore_pos < 0 or stretch_pos < 0
            or not (signature_pos < language_pos < restore_pos < stretch_pos)):
        fail("Restore Defaults must follow Language before the trailing Settings stretch")
    ...
    if "FILEVERSION 3,2,28,0" not in app_rc or '"V3.2.28\\0"' not in app_rc:
