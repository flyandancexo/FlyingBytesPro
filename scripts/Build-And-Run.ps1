...
try {
    ...
    Write-Banner 'Checking bundled USBasp driver support'
    $PrepareLibwdi = Join-Path $ProjectRoot 'scripts\Prepare-Libwdi.ps1'
    Require-File $PrepareLibwdi 'bundled libwdi check script'
    & $PrepareLibwdi -ProjectRoot $ProjectRoot
    $LibwdiRoot = Join-Path $ProjectRoot 'resources\libwdi'
    Require-File (Join-Path $LibwdiRoot 'include\libwdi.h') 'Prepared libwdi header'
    Require-File (Join-Path $LibwdiRoot 'lib\libwdi.a') 'Prepared libwdi static library'
    ...
    Get-ChildItem -LiteralPath $LibwdiRoot -Filter 'LICENSE.*.txt' -File -ErrorAction SilentlyContinue |
        ForEach-Object { Copy-Item -LiteralPath $_.FullName -Destination $DistDir -Force }

    # Developer documentation is source-only. It must never be deployed with the portable release.
    foreach ($DeveloperDocFolder in @('documentation', '_Doc')) {
        $UnexpectedDocPath = Join-Path $DistDir $DeveloperDocFolder
        if (Test-Path -LiteralPath $UnexpectedDocPath) {
            throw "Developer documentation was copied into the portable release: $UnexpectedDocPath"
        }
    }
    ...
    Write-Banner 'Removing temporary build files'
    if (Test-Path -LiteralPath $BuildDir) {
        Remove-Item -LiteralPath $BuildDir -Recurse -Force
    }
    if (Test-Path -LiteralPath $BuildDir) {
        throw "Temporary build directory could not be removed: $BuildDir"
    }
    Write-Host 'PASS: temporary compiler files removed; dist is the only release output.'
